...
async function calculateWeekRate(habits: Habit[]): Promise<number> {
  const today = todayKey();
  const weekDays = buildCurrentWeekDays().filter((day) => day.dateKey <= today);
  const active = habits.filter((habit) => !habit.isPaused);
  const groups = await Promise.all(active.map((habit) => listCheckInsForHabit(habit.id)));
  let scheduled = 0;
  let completed = 0;
  active.forEach((habit, index) => {
    for (const day of weekDays) {
      ...
      scheduled += 1;
      if (groups[index].some((checkIn) => checkIn.date === day.dateKey && checkIn.status === "completed")) {
        completed += 1;
      }
    }
  });
...
function HabitsHeader() {
  ...
  return <View style={{ flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
    <View style={{ flex: 1, gap: 4 }}>
      ...
      <AppText variant="body" tone="muted">岛上的日常角落</AppText>
    </View>
...
function HabitStats({ activeCount, weekRate }: { activeCount: number; weekRate: number }) {
  const { colors } = useTheme();
  const numberStyle = { fontSize: 26, lineHeight: 32, letterSpacing: numberLetterSpacing, fontFamily: "Outfit_800ExtraBold" };
  return <View style={{ flexDirection: "row", gap: 9 }}>
    <View style={{ flex: 1, borderRadius: 15, backgroundColor: colors.surfaceTint, paddingHorizontal: 12, paddingVertical: 11, gap: 4 }}>
      <AppText variant="small" tone="primary" style={{ fontWeight: "800", fontSize: 12, lineHeight: 16 }}>进行中</AppText>
      <AppText variant="title" tone="primary" style={numberStyle}>{activeCount}</AppText>
    </View>
    <View style={{ flex: 1, borderRadius: 15, backgroundColor: colors.partnerSurface, paddingHorizontal: 12, paddingVertical: 11, gap: 4 }}>
      <AppText variant="small" style={{ color: colors.partnerInk, fontWeight: "800", fontSize: 12, lineHeight: 16 }}>本周完成率</AppText>
      <AppText variant="title" style={[numberStyle, { color: colors.partnerInk }]}>{weekRate}%</AppText>
    </View>
  </View>;
}

function HabitTags({ habit, index }: { habit: Habit; index: number }) {
  const { colors } = useTheme();
  ...
  return <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
    <View style={{ borderRadius: 999, backgroundColor: colors.surfaceTint, paddingHorizontal: 9, paddingVertical: 4 }}>
      <AppText variant="small" tone="primary" style={{ fontWeight: "800", fontSize: 12, lineHeight: 16 }}>+10 XP</AppText>
    </View>
    {habit.trackType === "numeric" ? <HabitTag label="数值" background={colors.candySkySurface} color={colors.candySkyInk} />
      : habit.frequency.type === "weekdays" ? <HabitTag label="工作日" background={colors.candyOrangeSurface} color={colors.candyOrangeInk} /> : null}
    <HabitTag label={CORNER_NAMES[index % CORNER_NAMES.length]} background={colors[pair.bg]} color={colors[pair.fg]} />
  </View>;
}

function HabitTag({ label, background, color }: { label: string; background: string; color: string }) {
  return <View style={{ borderRadius: 999, backgroundColor: background, paddingHorizontal: 9, paddingVertical: 4 }}>
    <AppText variant="small" style={{ color, fontWeight: "800", fontSize: 12, lineHeight: 16 }}>{label}</AppText>
  </View>;
}

function HabitListItem({ habit, index }: { habit: Habit; index: number }) {
  const { colors } = useTheme();
  const pair = CHIP_PALETTE[index % CHIP_PALETTE.length];
  return <Pressable
  ...
  >
    ...
    <View style={{ flex: 1, gap: 6 }}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
        ...
        <Badge label={habit.isPaused ? "已暂停" : "活跃"} tone={habit.isPaused ? "muted" : "success"} />
      </View>
      <AppText variant="small" tone="faint" numberOfLines={1}>
        {frequencyLabel(habit)}{habit.reminderTime ? ` · ${habit.reminderTime} 提醒` : " · 无提醒"}
      </AppText>
      <HabitTags habit={habit} index={index} />
    </View>
  </Pressable>;
}
...
function HabitList({ habits }: { habits: Habit[] }) {
  if (habits.length === 0) {
    return <EmptyState fill title="岛上还没有角落" body="先新增一个习惯，或点右上角 ✨ 用 AI 对话生成分阶段计划。" />;
  }
  return <View style={{ gap: 8 }}>{habits.map((habit, index) => (
...
export default function HabitsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setHabits(all);
    setWeekRate(await calculateWeekRate(all));
  }, []);
  ...
  return (
    <Screen>
      <HabitsHeader />
      <HabitStats activeCount={activeCount} weekRate={weekRate} />
      {activeCount > 7 ? (
