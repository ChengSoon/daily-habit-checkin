...
OPENAI_MODEL=<模型名>          # 兜底模型，客户端可按请求覆盖
...
如果还是 `http://192.168.x.x:8787` 这类局域网 IP + 明文 HTTP，打进正式包后出了家门连不上，且生产构建禁明文 HTTP。改为：
...
- `apiKey`：`aiClient.ts` 会读它塞进 `x-api-key` 头调 AI 端点。必须与 `.env.dev` / `.env.prod` 中对应环境的 `API_KEY` 一致，否则 AI 生成计划会 401。
...
- `R2_PUBLIC_BASE`：R2 公开访问域名，例如 `https://your-cdn.example.com`
...
- [ ] `.env.prod` 已填强 `POSTGRES_PASSWORD` / `JWT_SECRET` / `API_KEY` / `OPENAI_API_KEY`
