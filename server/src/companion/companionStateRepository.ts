...
export function createCompanionStateRepository(options: StateRepositoryOptions = {}) {
  ...
  return {
      ...
      return transact(async (client) => {
        ...
        const selected = await client.query<MemberStateRow>(
          `SELECT pet_visible, proactive_mode, delivery_date, ordinary_count,
           ...
           FROM companion_member_state
           WHERE space_id = $1 AND account_id = $2 FOR UPDATE`,
        ...
        if (
          ...
          current.proactiveMode === "restrained" &&
          currentCount >= 1
        ) {
        ...
        await client.query(
          `UPDATE companion_member_state SET
             delivery_date = $3, ordinary_count = $4, last_ordinary_at = $5,
             recent_fingerprints = $6::jsonb, updated_at = now()
           WHERE space_id = $1 AND account_id = $2`,
          [
            input.spaceId,
            input.accountId,
            result.next.deliveryDate,
            result.next.ordinaryCount,
            result.next.lastOrdinaryAt,
            JSON.stringify(result.next.recentFingerprints)
          ]
        );
        return result;
      });
    ...
    async getMemberState(spaceId: string, accountId: string): Promise<MemberCompanionState> {
      await ensureMemberState(db, spaceId, accountId);
      const selected = await db.query<MemberStateRow>(
        `SELECT pet_visible, proactive_mode, delivery_date, ordinary_count,
                last_ordinary_at, recent_fingerprints, last_active_at
         FROM companion_member_state WHERE space_id = $1 AND account_id = $2`,
        [spaceId, accountId]
      );
    ...
    async updateMemberPreferences(
      ...
      await db.query(
        `INSERT INTO companion_member_state
           (space_id, account_id, pet_visible, proactive_mode)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (space_id, account_id) DO UPDATE SET
           pet_visible = excluded.pet_visible,
    ...
    async awardBond(
      ...
      sourceKey: string,
      points: number
    ): Promise<BondState & { awarded: boolean }> {
      return transact(async (client) => {
        const inserted = await client.query<{ source_key: string }>(
          `INSERT INTO companion_bond_events (space_id, source_key, points)
           VALUES ($1, $2, $3)
           ON CONFLICT (space_id, source_key) DO NOTHING
           RETURNING source_key`,
          [spaceId, sourceKey, points]
        );
        await client.query(
          `INSERT INTO companion_space_state (space_id)
           VALUES ($1) ON CONFLICT (space_id) DO NOTHING`,
          [spaceId]
        );
        const selected = await client.query<{ bond_points: number; bond_stage: BondStage }>(
          `SELECT bond_points, bond_stage FROM companion_space_state
        ...
        const next = nextBondState(
          { points: current.bond_points, seenSource: inserted.rows.length === 0 },
          points
        );
        if (!next.awarded) return next;
        await client.query(
          `UPDATE companion_space_state
           SET bond_points = $2, bond_stage = $3, updated_at = now()
           WHERE space_id = $1`,
    ...
    async getBondState(spaceId: string): Promise<BondState> {
      await db.query(
        `INSERT INTO companion_space_state (space_id)
         VALUES ($1) ON CONFLICT (space_id) DO NOTHING`,
        [spaceId]
      ...
      const result = await db.query<{ bond_points: number; bond_stage: BondStage }>(
        `SELECT bond_points, bond_stage FROM companion_space_state WHERE space_id = $1`,
        [spaceId]
      );
      return {
        points: result.rows[0]?.bond_points ?? 0,
        stage: result.rows[0]?.bond_stage ?? "first_meeting"
      };
    },
